...
#     prospect_table_fields = {f.fieldname for f in prospect.meta.get_table_fields()}

#     # Safe field setter - skips table fields and invalid Link/Select values.
...
#         if prospect.get(prospect_field) in (None, "", []):
...
#                 continue
#             if overwrite:
#                 safe_set(fieldname, getattr(source, fieldname))
#             else:
#                 fill_if_empty(fieldname, getattr(source, fieldname))

#     # Opportunity values are the latest values in this flow.
#     copy_matching_fields(opportunity, overwrite=True)

#     if lead:
#         # Linked Lead fills any blanks that Opportunity does not have.
#         copy_matching_fields(lead)

#         # Lead fields with different names on Prospect.
#         fill_if_empty("custom_contact_number",                getattr(lead, "custom_contact_person_phone_number", None))
#         fill_if_empty("custom_product",                       getattr(lead, "custom_product_name", None))
#         fill_if_empty("custom_designation",                   getattr(lead, "custom_designation", None))
#         fill_if_empty("custom_contact_person_email_id",       getattr(lead, "custom_contact_person_email_id", None))
...
#     # Link Opportunity without triggering ERPNext's update_prospect hook.
#     frappe.db.set_value(
#         "Opportunity",
#         opportunity.name,
#         {
